...
export function QuoteBuilder({
  ...
  function add() {
    setRows((rs) => [
      ...rs,
      { key: newKey(), description: "", qty: "1", unit: "m²", unitPrice: "0" },
    ]);
  }
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
              ...
              <tr key={row.key} className="border-b border-slate-100">
                ...
                <td className="px-2 py-1.5">
                  <Select
                    ...
                    disabled={!editable}
                    onValueChange={(v) =>
                      update(row.key, { unit: v as QuoteLineUnit })
                    }
                  >
      ...
      {/* Totals + save */}
      <div className="flex flex-col gap-4 border-t border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <dl className="w-full max-w-[240px] space-y-1 text-sm">
          ...
          <div
            className={cn(
              "flex justify-between rounded-md bg-slate-800 px-3 py-1.5 text-white",
            )}
          >
